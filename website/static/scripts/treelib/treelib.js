...
//----------------------------------------------------------------------------------------
function drawLine(svg_id, p0, p1, node_id, anc_id)
{
	...
	else
	{
		...
		line.setAttribute('class','colorizable');
		line.setAttribute('vector-effect','non-scaling-stroke');
		line.setAttribute('stroke-linecap','square');
		line.setAttribute('style','stroke:black;stroke-width:1;');
		line.setAttribute('d', linePath(p0, p1));
		line.setAttribute('node_id', node_id);
		line.setAttribute('anc_id', anc_id);
		var svg = document.getElementById(svg_id);
		svg.appendChild(line);

		//invisible line to host event (fat line is most easier to select)
		var fatline = document.createElementNS('http://www.w3.org/2000/svg','path');
		fatline.setAttribute('vector-effect','non-scaling-stroke');
		fatline.setAttribute("class", "overable")
		fatline.setAttribute('stroke-linecap','square');
		fatline.setAttribute('style','stroke:gray;stroke-width:10;opacity:0;');
		fatline.setAttribute('d', linePath(p0, p1));
		fatline.setAttribute('node_id', node_id);
		fatline.setAttribute('anc_id', anc_id);
		svg.appendChild(fatline);
	}
}

//----------------------------------------------------------------------------------------
function drawText(svg_id, p, string, p1)
{
	var text = document.createElementNS('http://www.w3.org/2000/svg','text');
	//newLine.setAttribute('id','node' + p.id);
	text.setAttribute('style','alignment-baseline:middle');
	text.setAttribute("class", "idLabel")
	text.setAttribute('x', p['x']-((p['x']-p1['x'])/1.7));
	text.setAttribute('y', p['y']-1.5);
...
function drawLeafText(svg_id, p, string)
{
	var text = document.createElementNS('http://www.w3.org/2000/svg','text');
	//newLine.setAttribute('id','node' + p.id);
	text.setAttribute('style','alignment-baseline:middle');
...
// specific to classic tree without vertical lines...

function drawTextforTree(svg_id, p, string, p1) {
	var text = document.createElementNS('http://www.w3.org/2000/svg', 'text');
	//newLine.setAttribute('id','node' + p.id);
	text.setAttribute('style', 'alignment-baseline:middle');
	text.setAttribute("class", "idLabel")
	text.setAttribute('x', p['x'] - ((p['x'] - p1['x']) / 1.7));
	//console.log(p['y'], p1['y'])
	text.setAttribute('y', ((p['y'] + p1['y'])/2));
...
//----------------------------------------------------------------------------------------
TreeDrawer.prototype.DrawLeaf = function(p, id)
{
	var p0 = p.xy
	var anc = p.ancestor;
	var node_id = p.id
	...
	if (anc)
	{
		...
		drawLine(this.settings.svg_id, p0, p1, node_id, anc_id);
		drawTextforTree(this.settings.svg_id, p0, id, p1);
	}
}

//----------------------------------------------------------------------------------------
TreeDrawer.prototype.DrawInternal = function(p, id)
{
	var p0 = p.xy
	var anc = p.ancestor;
	var node_id = p.id
...
//----------------------------------------------------------------------------------------
RectangleTreeDrawer.prototype.DrawLeaf = function(p, id)
{
	var p0 = p.xy
	var p1 = [];
	var anc = p.ancestor;
	...
	if (anc)
	{
		...
		drawLine(this.settings.svg_id, p0, p1, node_id, anc_id);
		drawText(this.settings.svg_id, p0, id, p1);
	}
}

//----------------------------------------------------------------------------------------
RectangleTreeDrawer.prototype.DrawInternal = function(p, id)
{
	var p0 = [];
	var p1 = [];
...
//----------------------------------------------------------------------------------------
CircleTreeDrawer.prototype.DrawLeaf = function(p, id)
{
...
//----------------------------------------------------------------------------------------
CircleTreeDrawer.prototype.DrawInternal = function(p, id)
{
	var p0 = [];
	var p1 = [];
...
//----------------------------------------------------------------------------------------
RadialTreeDrawer.prototype.DrawLeaf = function(p, id)
{
	var p0 = p.xy;
...
//----------------------------------------------------------------------------------------
RadialTreeDrawer.prototype.DrawInternal = function(p, id)
{
	var p0 = p.xy;
	var anc = p.ancestor;
	var node_id = p.id
